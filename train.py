...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target in val_loader:
            ...
            iou = iou_score(output, target)
            dice = dice_coef(output, target)
            accuracy = accuracy_score(target.cpu().numpy().ravel(), np.round(torch.sigmoid(output).cpu().numpy().ravel()))
...
def main():
    ...
    # Directory of Image, Mask folder generated from the preprocessing stage ###
                                                     #
    IMAGE_DIR = '/content/drive/MyDrive/mini_project/LIDC-IDRI-Preprocessing/data/Image'                                       #
    MASK_DIR = '/content/drive/MyDrive/mini_project/LIDC-IDRI-Preprocessing/data/Mask'                                         #
    #Meta Information                                                          #
    meta = pd.read_csv('/content/drive/MyDrive/mini_project/LIDC-IDRI-Preprocessing/data/Meta/meta.csv')                    #
    ############################################################################
    ...
    train_image_paths=train_image_paths[:4000]
    train_mask_paths=train_mask_paths[:4000]
    ...
    val_image_paths=val_image_paths[:1000]
    val_mask_paths=val_mask_paths[:1000]
    print("*"*50)
    ...
    for epoch in range(config['epochs']):
        ...
        val_accuracy = accuracy(val_log['pred'], val_log['target'])

        print('Training epoch [{}/{}], Training BCE loss:{:.4f}, Training DICE:{:.4f}, Training IOU:{:.4f}, Validation BCE loss:{:.4f}, Validation Dice:{:.4f}, Validation IOU:{:.4f}, Validation accuracy: {:.4f}'.format(
        epoch + 1, config['epochs'], train_log['loss'], train_log['dice'], train_log['iou'], val_log['loss'], val_log['dice'], val_log['iou'], val_accuracy))


        print('Training epoch [{}/{}], Training BCE loss:{:.4f}, Training DICE:{:.4f}, Training IOU:{:.4f}, Validation BCE loss:{:.4f}, Validation Dice:{:.4f}, Validation IOU:{:.4f},accuracy:{:.4f}'.format(
            epoch + 1, config['epochs'], train_log['loss'], train_log['dice'], train_log['iou'], val_log['loss'], val_log['dice'],val_log['iou'],val_log['accuracy']))
